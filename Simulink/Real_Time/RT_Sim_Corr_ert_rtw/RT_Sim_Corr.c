...
#include "RT_Sim_Corr_dt.h"

/* Block signals (default storage) */
BlockIO_RT_Sim_Corr RT_Sim_Corr_B;

/* Block states (default storage) */
D_Work_RT_Sim_Corr RT_Sim_Corr_DWork;

/* Real-time model */
RT_MODEL_RT_Sim_Corr RT_Sim_Corr_M_;
RT_MODEL_RT_Sim_Corr *const RT_Sim_Corr_M = &RT_Sim_Corr_M_;
...
static void rate_scheduler(void)
{
   ...
   */
  (RT_Sim_Corr_M->Timing.TaskCounters.TID[2])++;
  if ((RT_Sim_Corr_M->Timing.TaskCounters.TID[2]) > 49) {/* Sample time: [0.2s, 0.0s] */
    RT_Sim_Corr_M->Timing.TaskCounters.TID[2] = 0;
  }
}

/* Model step function */
void RT_Sim_Corr_step(void)
{
  ...
  srClearBC(RT_Sim_Corr_DWork.BalanceandDriveControl_SubsysRa);

  /* Reset subsysRan breadcrumbs */
  srClearBC(RT_Sim_Corr_DWork.GyroCalibration_SubsysRanBC);

  /* FromWorkspace: '<S2>/FromWs' */
  {
    real_T *pDataValues = (real_T *) RT_Sim_Corr_DWork.FromWs_PWORK.DataPtr;
    real_T *pTimeValues = (real_T *) RT_Sim_Corr_DWork.FromWs_PWORK.TimePtr;
    int_T currTimeIndex = RT_Sim_Corr_DWork.FromWs_IWORK.PrevIndex;
    real_T t = RT_Sim_Corr_M->Timing.t[0];
    ...
    RT_Sim_Corr_DWork.FromWs_IWORK.PrevIndex = currTimeIndex;
   ...
   */
  if (RT_Sim_Corr_P.ManualSwitch_CurrentSetting == 1) {
    rtb_Saturation3 = RT_Sim_Corr_P.Constant_Value;
  }
   ...
   *  Switch: '<S21>/Switch3'
   */
  if (RT_Sim_Corr_P.Constant6_Value_hyqvqn5fwb != 0.0F) {
    RT_Sim_Corr_B.counter_value = RT_Sim_Corr_P.Constant6_Value;
  } else {
    if (RT_Sim_Corr_P.Constant5_Value_jhrxeiwmg5 != 0.0F) {
      /* Switch: '<S21>/Switch3' incorporates:
       *  Constant: '<S6>/Constant7'
       */
      u0 = RT_Sim_Corr_P.Constant7_Value;
    } else {
      /* Switch: '<S21>/Switch3' incorporates:
       *  Constant: '<S21>/Constant5'
       */
      u0 = RT_Sim_Corr_P.Constant5_Value;
    }
     ...
     */
    if ((u0 < RT_Sim_Corr_P.Constant8_Value) || rtIsNaNF
        (RT_Sim_Corr_P.Constant8_Value)) {
      RT_Sim_Corr_B.counter_value = u0;
    } else {
      RT_Sim_Corr_B.counter_value = RT_Sim_Corr_P.Constant8_Value;
    }
   ...
   */
  Compare = (RT_Sim_Corr_B.counter_value >= RT_Sim_Corr_P.time_start);
  ...
  if (!Compare) {
    if (1) {
       ...
       */
      RT_Sim_Corr_B.Sum1 = RT_Sim_Corr_P.Gain2_Gain * (real32_T)rtb_Encoder1_0 +
        RT_Sim_Corr_P.a_gc * RT_Sim_Corr_DWork.UnitDelay_DSTATE_cfkvxtl130;
    }

    if (RT_Sim_Corr_M->Timing.TaskCounters.TID[2] == 0) {
      /* S-Function (ev3_speaker): '<S20>/Speaker' incorporates:
       *  Constant: '<S5>/Constant'
       */
      playSoundFreqOnly(RT_Sim_Corr_P.Constant_Value_aljrxhz2pj,
    ...
    if (1) {
      ...
      RT_Sim_Corr_DWork.UnitDelay_DSTATE_cfkvxtl130 = RT_Sim_Corr_B.Sum1;
    }

    srUpdateBC(RT_Sim_Corr_DWork.GyroCalibration_SubsysRanBC);
  }
   ...
   */
  RT_Sim_Corr_DWork.DiscreteFilter_tmp = (RT_Sim_Corr_P.BATTERY -
    RT_Sim_Corr_P.DiscreteFilter_DenCoef[1] *
    RT_Sim_Corr_DWork.DiscreteFilter_states) /
    RT_Sim_Corr_P.DiscreteFilter_DenCoef[0];

  /* Outputs for Enabled SubSystem: '<S1>/Balance and Drive Control' incorporates:
   *  EnablePort: '<S3>/Enable'
   */
  RT_Sim_Corr_DWork.BalanceandDriveControl_MODE = Compare;
  if (RT_Sim_Corr_DWork.BalanceandDriveControl_MODE) {
    /* UnitDelay: '<S17>/Unit Delay' */
    RT_Sim_Corr_B.UnitDelay = RT_Sim_Corr_DWork.UnitDelay_DSTATE_hzuovbre5y;
     ...
     */
    if ((real32_T)fabs(RT_Sim_Corr_B.UnitDelay) >=
        RT_Sim_Corr_P.CompareToConstant1_const) {
      rtmSetStopRequested(RT_Sim_Corr_M, 1);
    }
     ...
     */
    if (RT_Sim_Corr_DWork.FixPtUnitDelay2_DSTATE != 0) {
      u0 = RT_Sim_Corr_B.Sum1;
    } else {
      u0 = RT_Sim_Corr_DWork.FixPtUnitDelay1_DSTATE;
    }
     ...
     *  S-Function (ev3_gyro_sensor): '<S1>/Gyro Sensor1'
     */
    rtb_Sum2 = RT_Sim_Corr_P.Gain1_Gain_j0r0df4nzm * (real32_T)rtb_Encoder1_0 +
      RT_Sim_Corr_P.a_gd * u0;
     ...
     *  Sum: '<S8>/Sum6'
     */
    RT_Sim_Corr_B.Gain = ((RT_Sim_Corr_P.deg2rad_Gain * (real32_T)rtb_Encoder1_1
      + RT_Sim_Corr_B.UnitDelay) + (RT_Sim_Corr_P.deg2rad1_Gain * (real32_T)
      rtb_Encoder1_2 + RT_Sim_Corr_B.UnitDelay)) * RT_Sim_Corr_P.Gain_Gain;
     ...
     */
    rtb_Sum = RT_Sim_Corr_P.Gain3_Gain_hka0xvmyzz * RT_Sim_Corr_B.Gain +
      RT_Sim_Corr_P.a_d * RT_Sim_Corr_DWork.UnitDelay_DSTATE_orvhqhlbcp;

    /* Gain: '<S16>/Gain' incorporates:
     *  Sum: '<S16>/Sum'
     *  UnitDelay: '<S16>/Unit Delay'
     */
    RT_Sim_Corr_B.Gain_oopwalytwu = (rtb_Sum -
     ...
     *  Sum: '<S8>/Sum2'
     */
    RT_Sim_Corr_B.psidot = ((real32_T)rtb_Encoder1_0 - rtb_Sum2) *
      RT_Sim_Corr_P.deg2rad2_Gain;

    /* Sum: '<S17>/Sum' incorporates:
     *  Gain: '<S17>/Gain'
     */
    rtb_Sum_nvdedzgoqp = RT_Sim_Corr_P.ts1 * RT_Sim_Corr_B.psidot +
     ...
     *  Sum: '<S12>/Sum2'
     */
    rtb_Saturation3_tmp = RT_Sim_Corr_B.Sum /
    ...
    /* Gain: '<S7>/Gain1' */
    rtb_Saturation3 *= RT_Sim_Corr_P.Gain1_Gain;
     ...
     *  Gain: '<S13>/Gain1'
     */
    rtb_Saturation3 = RT_Sim_Corr_P.pwm_offset * tmp_0 + RT_Sim_Corr_P.pwm_gain *
      rtb_Saturation3;

    /* Saturate: '<S7>/Saturation2' */
    if (rtb_Saturation3 > RT_Sim_Corr_P.Saturation2_UpperSat) {
      rtb_Saturation3 = RT_Sim_Corr_P.Saturation2_UpperSat;
    } else {
      if (rtb_Saturation3 < RT_Sim_Corr_P.Saturation2_LowerSat) {
        rtb_Saturation3 = RT_Sim_Corr_P.Saturation2_LowerSat;
      }
    }
    ...
    /* Gain: '<S7>/Gain3' */
    rtb_Saturation3 *= RT_Sim_Corr_P.Gain3_Gain;
    ...
    if (rtb_Saturation3 > RT_Sim_Corr_P.Saturation3_UpperSat) {
      rtb_Saturation3 = RT_Sim_Corr_P.Saturation3_UpperSat;
    } else {
      if (rtb_Saturation3 < RT_Sim_Corr_P.Saturation3_LowerSat) {
        rtb_Saturation3 = RT_Sim_Corr_P.Saturation3_LowerSat;
      }
    }
    ...
    /* S-Function (ev3_motor): '<S11>/Motor' */
    setMotor(&tmp, 3U, 2U);
    srUpdateBC(RT_Sim_Corr_DWork.BalanceandDriveControl_SubsysRa);
  }
  ...
  if (RT_Sim_Corr_DWork.BalanceandDriveControl_MODE) {
    ...
    RT_Sim_Corr_DWork.UnitDelay_DSTATE_hzuovbre5y = rtb_Sum_nvdedzgoqp;

    /* Update for UnitDelay: '<S19>/FixPt Unit Delay2' incorporates:
     *  Constant: '<S19>/FixPt Constant'
     */
    RT_Sim_Corr_DWork.FixPtUnitDelay2_DSTATE = RT_Sim_Corr_P.FixPtConstant_Value;

    /* Update for UnitDelay: '<S19>/FixPt Unit Delay1' */
    RT_Sim_Corr_DWork.FixPtUnitDelay1_DSTATE = rtb_Sum2;

    /* Update for UnitDelay: '<S18>/Unit Delay' */
    RT_Sim_Corr_DWork.UnitDelay_DSTATE_orvhqhlbcp = rtb_Sum;

    /* Update for UnitDelay: '<S16>/Unit Delay' */
    RT_Sim_Corr_DWork.UnitDelay_DSTATE_mjna1kxy33 = rtb_Sum;
  }
  ...
  {                                    /* Sample time: [0.0s, 0.0s] */
    rtExtModeUpload(0, (real_T)RT_Sim_Corr_M->Timing.t[0]);
  }

  {                                    /* Sample time: [0.004s, 0.0s] */
    rtExtModeUpload(1, (real_T)(((RT_Sim_Corr_M->Timing.clockTick1+
      RT_Sim_Corr_M->Timing.clockTickH1* 4294967296.0)) * 0.004));
  }

  if (RT_Sim_Corr_M->Timing.TaskCounters.TID[2] == 0) {/* Sample time: [0.2s, 0.0s] */
    rtExtModeUpload(2, (real_T)(((RT_Sim_Corr_M->Timing.clockTick2+
      RT_Sim_Corr_M->Timing.clockTickH2* 4294967296.0)) * 0.2));
  }

  /* signal main to stop simulation */
  {                                    /* Sample time: [0.0s, 0.0s] */
    if ((rtmGetTFinal(RT_Sim_Corr_M)!=-1) &&
        !((rtmGetTFinal(RT_Sim_Corr_M)-RT_Sim_Corr_M->Timing.t[0]) >
          RT_Sim_Corr_M->Timing.t[0] * (DBL_EPSILON))) {
      rtmSetErrorStatus(RT_Sim_Corr_M, "Simulation finished");
    }

    if (rtmGetStopRequested(RT_Sim_Corr_M)) {
      rtmSetErrorStatus(RT_Sim_Corr_M, "Simulation finished");
    }
  }
   ...
   */
  if (!(++RT_Sim_Corr_M->Timing.clockTick0)) {
    ++RT_Sim_Corr_M->Timing.clockTickH0;
  }

  RT_Sim_Corr_M->Timing.t[0] = RT_Sim_Corr_M->Timing.clockTick0 *
    RT_Sim_Corr_M->Timing.stepSize0 + RT_Sim_Corr_M->Timing.clockTickH0 *
    RT_Sim_Corr_M->Timing.stepSize0 * 4294967296.0;

  {
     ...
     */
    RT_Sim_Corr_M->Timing.clockTick1++;
    if (!RT_Sim_Corr_M->Timing.clockTick1) {
      RT_Sim_Corr_M->Timing.clockTickH1++;
    }
  }
  ...
  if (RT_Sim_Corr_M->Timing.TaskCounters.TID[2] == 0) {
     ...
     */
    RT_Sim_Corr_M->Timing.clockTick2++;
    if (!RT_Sim_Corr_M->Timing.clockTick2) {
      RT_Sim_Corr_M->Timing.clockTickH2++;
    }
  }
...
void RT_Sim_Corr_initialize(void)
{
  /* Registration code */
  ...
  (void) memset((void *)RT_Sim_Corr_M, 0,
                sizeof(RT_MODEL_RT_Sim_Corr));

  {
    /* Setup solver object */
    rtsiSetSimTimeStepPtr(&RT_Sim_Corr_M->solverInfo,
                          &RT_Sim_Corr_M->Timing.simTimeStep);
    rtsiSetTPtr(&RT_Sim_Corr_M->solverInfo, &rtmGetTPtr(RT_Sim_Corr_M));
    rtsiSetStepSizePtr(&RT_Sim_Corr_M->solverInfo,
                       &RT_Sim_Corr_M->Timing.stepSize0);
    rtsiSetErrorStatusPtr(&RT_Sim_Corr_M->solverInfo, (&rtmGetErrorStatus
      (RT_Sim_Corr_M)));
    rtsiSetRTModelPtr(&RT_Sim_Corr_M->solverInfo, RT_Sim_Corr_M);
  }

  rtsiSetSimTimeStep(&RT_Sim_Corr_M->solverInfo, MAJOR_TIME_STEP);
  rtsiSetSolverName(&RT_Sim_Corr_M->solverInfo,"FixedStepDiscrete");
  rtmSetTPtr(RT_Sim_Corr_M, &RT_Sim_Corr_M->Timing.tArray[0]);
  rtmSetTFinal(RT_Sim_Corr_M, -1);
...

  {
    ...
    RT_Sim_Corr_M->extModeInfo = (&rt_ExtModeInfo);
    rteiSetSubSystemActiveVectorAddresses(&rt_ExtModeInfo, systemRan);
    systemRan[0] = &rtAlwaysEnabled;
    systemRan[1] = (sysRanDType *)
      &RT_Sim_Corr_DWork.BalanceandDriveControl_SubsysRa;
    systemRan[2] = (sysRanDType *)&RT_Sim_Corr_DWork.GyroCalibration_SubsysRanBC;
    systemRan[3] = &rtAlwaysEnabled;
    systemRan[4] = &rtAlwaysEnabled;
    systemRan[5] = &rtAlwaysEnabled;
    rteiSetModelMappingInfoPtr(RT_Sim_Corr_M->extModeInfo,
      &RT_Sim_Corr_M->SpecialInfo.mappingInfo);
    rteiSetChecksumsPtr(RT_Sim_Corr_M->extModeInfo,
                        RT_Sim_Corr_M->Sizes.checksums);
    rteiSetTPtr(RT_Sim_Corr_M->extModeInfo, rtmGetTPtr(RT_Sim_Corr_M));
  }

  /* block I/O */
  (void) memset(((void *) &RT_Sim_Corr_B), 0,
                sizeof(BlockIO_RT_Sim_Corr));

  /* states (dwork) */
  (void) memset((void *)&RT_Sim_Corr_DWork, 0,
                sizeof(D_Work_RT_Sim_Corr));
  ...
  /* data type transition information */
  {
    ...
    (void) memset((char_T *) &dtInfo, 0,
                  sizeof(dtInfo));
    RT_Sim_Corr_M->SpecialInfo.mappingInfo = (&dtInfo);
  ...
  /* Start for FromWorkspace: '<S2>/FromWs' */
  {
    ...
    RT_Sim_Corr_DWork.FromWs_PWORK.TimePtr = (void *) pTimeValues0;
    RT_Sim_Corr_DWork.FromWs_PWORK.DataPtr = (void *) pDataValues0;
    RT_Sim_Corr_DWork.FromWs_IWORK.PrevIndex = 0;
  }
...
void RT_Sim_Corr_terminate(void)
{
